...
test_that("archive_overview() returns a plot with the correct time frame", {
  output_overview <- archive_overview(homepage = "nytimes.com", startDate = "2018-06-01", endDate = "Dec/01/2018")
...
test_that("archive_overview() returns a plot in gtable class", {
  output_overview <- archive_overview(homepage = "nytimes.com", startDate = "2018-06-01", endDate = "Dec/01/2019")
...
test_that("archive_overview() only creates plot for existing URLs", {
  expect_error(archive_overview("www.independent..co.uk", "20Sep2014", "20Sep2015"), "Could not resolve host")
})

#Check if start date is character
test_that("archive_overview() only takes character vectors as startDate", {
  expect_error(archive_overview("nytimes.com", 2015-01-01, "20Sep2015"), "startDate is not a character vector")
})

#Check if end date is character
test_that("archive_overview() only takes character vectors as endDate", {
  expect_error(archive_overview("nytimes.com", "2015-01-01", 2015-05-31), "endDate is not a character vector")
})

#Check if startdate is a date
test_that("archive_overview() only takes dates as startDate", {
  expect_error(archive_overview("nytimes.com", "date", "20Sep2015"), "startDate is not a date")
})

#Check if enddate is a date
test_that("archive_overview() only takes dates as startDate", {
  expect_error(archive_overview("nytimes.com", "2015-01-01", "date"), "endDate is not a date")
})

#Check if enddate is after startdate
test_that("archive_overview() needs endDate to be later than startDate", {
  expect_error(archive_overview("nytimes.com", "2016-01-01", "2015-05-31"), "startDate cannot be later")
})

#Check if enddate is not in the future
test_that("archive_overview() needs endDate to be not in the future", {
  expect_error(archive_overview("nytimes.com", "2016-01-01", "2055-05-31"), "endDate cannot be in the future")
})
...
test_that("archive_overview() needs homepage to be saved in the Internet Archive", {
  expect_error(archive_overview("https://cyprus-mail.com/2021/02/18/the-secret-helping-car-companies-stay-profitable/", "2016-01-01", "2016-05-31"), "Homepage has never been saved in the Internet Archive")
})
