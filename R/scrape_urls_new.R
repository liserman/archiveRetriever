...
for (i in (seq_len(length(Urls)-(startnum-1))+(startnum-1))) {
  ...
  # Stops
    ...
    if (collapse == FALSE & length(unique(sapply(data, length)))!=1) {
      ...
      warning(
        paste0(
          ...
          " '",
          Urls[i],
          "'. Number of elemens for paths differs. Please provide paths that result in a similar number of elements per Url or use the option collapse = TRUE. A preliminary output has been printed."
        )
      )
      return(output)
    }
